...
**W4実装時の追加検証結果**: trigram トークナイザは3文字未満の入力からは
トライグラムを1つも生成できないため、**1〜2文字のクエリは常にマッチ0件になる**
（modernc.org/sqlite で実機検証: 「返品」「配送」等の2文字クエリで確認）。
日本語では2文字語彙が非常に多い（返品/配送/料金/会社/天気...）ため、これを
未対応のまま出荷すると実用上致命的。対応として、正規化後のクエリが3文字未満の
場合は `LIKE '%query%'` へフォールバックする実装を追加（`internal/adapter/sqlite/fts_store.go`）。
また、3文字以上のMATCHクエリはユーザー入力をFTS5クエリ構文としてではなく
「フレーズリテラル」（ダブルクォートで囲みエスケープ）として渡すことで、
ハイフンや引用符を含む任意の入力でも構文エラーにならないようにしている。
